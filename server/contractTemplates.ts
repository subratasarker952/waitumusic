...
interface Signer {
  role: string; // "Applicant", "Assigned Admin", "Lawyer", "Professional", "Superadmin"
  fullName: string; // ফাইল এ text হিসেবে দেখানো হবে
  date?: string; // optional, আজকের date auto-fill করতে পারো
}
...
function generatePublishingContract(
  ...
  doc.text(
    '1. Wai\'tuMusic having its registered office at C/o Krystallion Incorporated, 31 Bath Estate, P.O. Box 1350, Roseau, Dominica: (the "Publisher")'
  );
  doc.moveDown();
  ...
  addSignatureSection(doc, data.signetures);

  return doc;
}

function generateAdministrationContract(
  doc: PDFKit.PDFDocument,
  data: ContractData
): PDFKit.PDFDocument {
  // Header
  ...
  doc.text(
    "(A) Wai'tuMusic C/o Krystallion Incorporated, P.O. Box 1350, Roseau, Dominica -"
  );
  ...
  doc.text(
    `of [Contact info]: Email: ${data.email} ; Phone: ${data.phoneNumber} `
  );
  doc.moveDown();
  ...
  doc.moveDown();
  doc.text(
    "• Administration Commission: 25% of Gross Earnings after Authorized Expenses"
  );
  doc.text("• Artist Fee: 75% of Gross Earnings after Authorized Expenses");
  doc.text(`• Term: ${data.termLength}`);
  doc.text("• Territory: The World");
  doc.text(
    "• Post-Term Commission: 25% for life of contracts secured during term"
  );
  doc.text(
    "• Services: Career development, deal negotiation, professional guidance"
  );
  doc.moveDown();
...
function generateManagementContract(
  doc: PDFKit.PDFDocument,
  data: ContractData
): PDFKit.PDFDocument {
  // Header
  ...
  doc.moveDown();
  doc.text(
    "• Management Commission: 25% of Gross Earnings after Authorized Expenses"
  );
  doc.text("• Artist Fee: 75% of Gross Earnings after Authorized Expenses");
  doc.text(`• Term: ${data.termLength} (auto-renewable)`);
  doc.text("• Territory: The World");
  doc.text(
    "• Post-Term Commission: 12.5% for life of contracts secured during term"
  );
  doc.text(
    "• Services: Exclusive personal music business representation and full management"
  );
...
function generateProfessionalServicesContract(
  doc: PDFKit.PDFDocument,
  data: ContractData
): PDFKit.PDFDocument {
  // Header
  doc
    .fontSize(16)
    .text("PROFESSIONAL SERVICES MANAGEMENT AGREEMENT", { align: "center" });
  doc.moveDown(2);
  ...
  doc.text(
    '1. Wai\'tuMusic having its registered office at C/o Krystallion Incorporated, 31 Bath Estate, P.O. Box 1350, Roseau, Dominica: (the "Company")'
  );
  doc.moveDown();
  ...
  const serviceTerms = getProfessionalServiceTerms(
    data.professionalType || "business"
  );
...
function getProfessionalServiceTerms(professionalType: string) {
  const terms = {
    legal: {
      scope:
        "The Professional shall provide legal counsel services including but not limited to contract review, intellectual property protection, rights management, legal compliance guidance, and representation in music industry legal matters.",
      obligations:
    ...
    marketing: {
      scope:
        "The Professional shall provide marketing and promotional services including digital marketing strategy, social media management, brand development, public relations campaign management, and audience engagement optimization.",
      obligations:
    ...
    business: {
      scope:
        "The Professional shall provide business consulting services including strategic planning, revenue optimization, partnership development, market analysis, and business development guidance for music industry participants.",
      obligations:
    ...
    financial: {
      scope:
        "The Professional shall provide financial consulting services including financial planning, revenue analysis, tax strategy, investment guidance, royalty management, and accounting services for music industry professionals.",
      obligations:
    ...
    brand: {
      scope:
        "The Professional shall provide brand management services including visual identity development, content strategy, image consulting, public relations, and brand positioning for artists and music industry professionals.",
      obligations:
        "• Develop comprehensive brand identity and style guidelines\n• Create and manage content strategies across multiple platforms\n• Monitor public perception and manage reputation\n• Coordinate public relations activities and media interactions\n• Ensure brand consistency across all touchpoints and communications",
    },
  };
...
function addSignatureSection(doc: PDFKit.PDFDocument, signers: Signer[]): void {
  doc.addPage();
  doc
    .fontSize(12)
    .text(
      "IN WITNESS WHEREOF, the parties hereto have executed this Agreement the day and year herein above first written:"
    );
  doc.moveDown(2);

  signers.forEach((signer) => {
    doc.text(`("${signer.role}")`);
    doc.moveDown();
    doc.text(`Full Name: ${signer.fullName}`);
    doc.text("Signature: ____________________");
    doc.text(`Date: ${signer.date ?? new Date().toLocaleDateString()}`);
    doc.moveDown(2);

    doc.text('("Witness")');
    doc.moveDown();
    doc.text("Full Name: ____________________");
    doc.text("Signature: ____________________");
  ...
  doc
    .fontSize(10)
    .text(
      "Note: Minors. If a minor (a person under 18 years old) enters into a contract, that person can \"disaffirm\" (void the contract) this Agreement at any time while he or she is a minor unless a contract is held to be for items necessary for life. As it's likely this isn't such a contract, the parents of a minor should co-sign this Agreement to make it legally valid. They are then bound by its terms."
    );
}

export function getContractTypeFromTier(
  tierId: number
...
export function getTierCommissions(tierId: number): {
  commission: string;
  postTermCommission?: string;
} {
  switch (tierId) {
